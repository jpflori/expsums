...
\label{cnj:kloofour}
For $a \in \GF{m}^*$ and $b \in \GF[4]{}^*$, the function $f_{a,b}$ is bent
if and only if $K_{m_1}(a) = 4$.
\end{conjecture}
...
functions $f_{a,b}$ for $a \in \GF{m_1}^*$ to extend results to $\GF{m_0}^*$.
\begin{lemma}
...
We now start splitting the Walsh transform of $f_{a,b}$ at $\omega \in \GF{m_0}$
...
We recall the main ingredients and results as
...
and yield a concise and easy to compute formula for the Walsh transform of $f_{a,b}$.
...
The main difference with the case $\nu = 1$ is that $3$ does now divide $2^{m_1}-1$ (in fact $2^{m_\nu}+1$) rather than $2^{m_1}+1$ and $\mulch[m_0]{u}$ does not depend on the value of $u_1$ (but only on that of $u_\nu$).
...
Define $\gamma \in \GF[4]{}^*$ by $\gamma = b \mulch[m_1]{w_2}^{-2}$
...
\Snu[2]
& =\sum_{u_1 \neq 1, \mulch[m_1]{u_1^2 + u_1^{-2}} = b \mulch[m_1]{w_2}^{-2}} \addch{\tr{m_1}{a \left( u_1^{-2}+ u_1^{2} \right)}} \nonumber \\
& = \sum_{u_1 \neq 1, \mulch[m_1]{u_1 + u_1^{-1}} = b \mulch[m_1]{w_2}^{-2}} \addch{\tr{m_1}{a \left( u_1 + u_1^{-1} \right)}} \nonumber \\
& = 2 \sum_{t \in \T_{m_1}^1, \mulch[m_1]{t} = b \mulch[m_1]{w_2}^{-2}} \addch{\tr{m_1}{a t}} \nonumber \\
...
Making the change of variable $x = rx$ and properties of the Dickson
polynomial $D_3$when $m_1$ is even show that for $\gamma = \alpha = \mulch[m_1]{r^{-1}}$:
\begin{align}
...
in particular because of the fact that the multiplicative and additive characters
of $\GF{m_1}$ act on different values, \eg
$r = \tr[m_1]{m_0}{w_1 u_1}$ (or $s = \tr[m_1]{m_0}{w_1^{-1} u_1}$) and $t = \tr[m_1]{m_0}{u_1}$.
...
Yet, experimental evidence suggests that the following conjecture is true.
\begin{conjecture}
\label{cnj:walshconj}
For $\nu = 2$, $a \in \GF{m_1}^*$ with $K_{m_1}(a) \equiv 1 \pmod{3}$
...
& - \left( 1 - \addch{\mystery} \right) 2^{m_2-1} \nonumber \\
& - \left( 1 - \addch{\tr{m_0}{a w_1^2}} \right) \frac{2^{m_2-1} - 1}{3} \enspace .
\end{align*}
...
The computation of $\Snu[2]$ was implemented in C and assembly%
...
PARI/GP for computation of $K_{m_1}(a)$,
and OpenMP for parallelization.
The computational cost of verifying the conjecture can be somewhat
leveraged by the fact that $\Snu[2]$ only depends on the cyclotomic
class of $a$ and is the same for $w_1$ and $w_1^-1$.
For a given couple $(a, w_1)$, computing naively $\Snu[2]$
to check the conjecture has time complexity of the order of $O(2^{m_1})$.
As there are $2^{m_1-1}$ values of $w_1$ to check for each $a$
and about $2^{m_1}/3$ values of $a$ to check, there is no hope to cover
all possible values when $m_1$ is large.
Still we did it
\begin{itemize}
\item completely for $m_2 = 3, 5, 7$,
\item for $i$ up to $3405$ where $a = z^i$
and $z$ is a primitive element of $\GF{m_1}$ for $m_2 = 9$.
\end{itemize}
