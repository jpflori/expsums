...
and from $\GF{m_i}^* \backslash \set{1}$ to $\T_{m_i}^0$
...
characters of $\GF{m}$ for any extension degree $m$.

The non-principal cubic multiplicative character $\mulch{}$ of $\GF{m}$
for any even extension degree $m$ is defined as
\[
\mulch{x} = x^{\frac{2^{m}-1}{3}} \enspace ,
\]
where $x \in \GF{m}$.
...
\begin{align*}
\sum_{u_1 \in U_1} \addch{\tr{m_0}{a u_1}} & = 1 - K_{m_1}(a) \enspace .
\end{align*}
...
\begin{align*}
C_{m1}(a, b) & = \sum_{x \in \GF{m_1}} \addch{\tr{m_1}{a x^3 + b x}}
\end{align*}
...
is that $C_{m_1}(a, a) = 0$ if and only if
\begin{itemize}
...
when $m_1$ is even.
\end{itemize}
Charpin \etal later deduced that both in the odd case~\cite{mod24}
...
When $a$ is a cube and $\tr[2]{m_1}{\alpha} = 0$, then $C_{m_1}(a, a) = 2^{m_2 + 1} \addch{\tr{m_1}{u_0^3}}$,
where $u_0$ is any solution to $u^4 + u = \alpha^4$,
that is $u_0 = \sum_{i=0}^{(m_2-3)/2} \alpha^{4^{2*i+2}} + \gamma$ for any $\gamma \in \GF[4]{}$.
...
The values of $C_{m_1}(a, a)$ computed by Carlitz~\cite{MR544577}
yield a concise and easy to compute formula for the Walsh spectrum
of $f_{a,b}$.
...
In the case where $K_{m_1}(a) \equiv 1 \pmod{3}$, which includes the case we are ultimately interested in, $K_{m_1}(a) = 4$, $C_{m_1}(a, a) = 0$ and the above equality becomes for $\gamma = 1/r = 1$:
\begin{align*}
